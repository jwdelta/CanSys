...
while [[ $# -gt 0 ]]; do
	...
	-pop) pop="$2"; shift; shift ;;
	-Cutoff_AF_pop) Cutoff_AF_pop="$2"; shift; shift ;;
...
# Check if pop is one of the allowed values
if [[ -n "$pop" ]]; then
    if [[ ! "$pop" =~ ^(ALL|AFR|AMR|ASJ|EAS|FIN|MID|NFE|SAS|OTH)$ ]]; then
	echo "Error: -pop must be one of ALL, AFR, AMR, ASJ, EAS, FIN, MID, NFE, SAS or OTH."
	exit 1
    fi
fi
...
Temp_ExecDir="${Output_Dir}/$(date +%s%N${RANDOM} | sha256sum | head -c 16).tmp"
...
annotate_variants() {
    ...
    # Run ANNOVAR
    $annovar_Dir/table_annovar.pl $vcf $annovar_Dir/humandb/ -buildver hg38 -out $sample -remove -protocol refGene,gnomad41_exome -operation g,f -nastring . -polish -vcfinput
    # Process annotation here
    if [[ -n "$pop" && -n "$Cutoff_AF_pop" ]]; then
	bash "$Pipeline_Dir/src/filter_by_popAF.sh" "${sample}.hg38_multianno.vcf" "$pop" "$Cutoff_AF_pop"
    fi
    
    if [[ -e "${sample}.hg38_multianno.filtered.vcf" ]]; then
	mv "${sample}.hg38_multianno.filtered.vcf" "${sample}.hg38_multianno.vcf"
    fi
